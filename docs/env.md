...
| `MESSAGE_INSERT_LOCK_MODE` | `serialized` (default): Redis + in-process queue serialize channel `POST /messages` inserts per `channel_id` to reduce hot btree/GIN page contention. `optimistic` (aliases **`off`**, **`false`**, **`none`**): skip that lock entirely; inserts run concurrently. Counted in `message_channel_insert_lock_total{result="optimistic_bypass"}`. |
...
| `MESSAGE_FANOUT_JOB_DONE_TTL_SEC` | Redis “fanout completed” marker TTL (default **86400** s, clamped **3600–604800**) so duplicate jobs skip **`PUBLISH`**. |
