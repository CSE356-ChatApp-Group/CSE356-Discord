...
**Channel `message:created`:** the app **defaults to** logical per-member user delivery, implemented internally via sharded Redis **`userfeed:<n>`** publishes that route to each socket’s **`user:<id>`** stream (see [`docs/GRADING-DELIVERY-SEMANTICS.md`](../docs/GRADING-DELIVERY-SEMANTICS.md)). **`deploy-staging.sh`** and **`deploy-prod.sh`** still re-apply recommended fanout/cache throughput knobs (`CHANNEL_MESSAGE_USER_FANOUT*`, `MESSAGE_USER_FANOUT_HTTP_BLOCKING`, `USER_FEED_SHARD_COUNT`, `WS_BOOTSTRAP_*`, list-cache TTLs) on deploy, but now **preserve** existing values for `WS_AUTO_SUBSCRIBE_MODE` and `READ_RECEIPT_DEFER_POOL_WAITING` (setting safe defaults only if missing). To intentionally change behavior long-term, update both the deploy script defaults and `/opt/chatapp/shared/.env`.
