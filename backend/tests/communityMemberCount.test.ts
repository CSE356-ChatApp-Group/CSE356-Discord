...
jest.mock("../src/messages/fanout/channelRealtimeFanout", () => ({
  invalidateCommunityChannelUserFanoutTargetsCache: jest
    .fn()
...
jest.mock("../src/utils/autoIpBan", () => ({
  recordAbuseStrikeFromRequest: jest.fn(),
}));

// ── Helpers ─────────────────────────────────────────────────────────────────────

jest.mock("../src/db/redisBatch", () => ({
  redisBatchHmget: jest.fn((client, key, fields) =>
    client.hmget(key, ...fields),
  ),
  redisBatchSrem: jest.fn((client, key, members) =>
    client.srem(key, ...members),
  ),
  redisBatchSadd: jest.fn((client, key, members) =>
...
beforeEach(() => {
  jest.clearAllMocks();
  jest.useFakeTimers();
  poolMock.poolStats.mockReturnValue({
    waiting: 0,
    total: 5,
    idle: 5,
    max: 25,
...
describe("incrCommunityMemberCount", () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    incrCommunityMemberCount =
      require("../src/communities/communityMemberCount").incrCommunityMemberCount;
  });
  ...
  it("calls HINCRBY +1 and SADD dirty via pipeline", async () => {
    ...
    expect(metricsMock.communityCountRedisUpdateTotal.inc).toHaveBeenCalledWith(
      { result: "ok" },
    );
  });
...
describe("decrCommunityMemberCount", () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    decrCommunityMemberCount =
      require("../src/communities/communityMemberCount").decrCommunityMemberCount;
  });
  ...
  it("clamps to 0 by calling HSET when HINCRBY returns negative", async () => {
    ...
    const clampPipeline = makePipeline([[null, "OK"]]);
    redisMock.pipeline
      .mockReturnValueOnce(pipeline)
      .mockReturnValueOnce(clampPipeline);
    ...
    expect(clampPipeline.hset).toHaveBeenCalledWith(
      "community:counts",
      "c-1",
      "0",
    );
...
describe("getCommunityMemberCountsFromRedis", () => {
  let getCommunityMemberCountsFromRedis: (
    ids: string[],
  ...
  it("maps present keys to counts and emits hit metrics", async () => {
    ...
    expect(metricsMock.communityCountCacheTotal.inc).toHaveBeenCalledWith({
      result: "hit",
    });
    expect(metricsMock.communityCountCacheTotal.inc).toHaveBeenCalledWith({
      result: "miss",
...
describe("runReconcile", () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    runReconcile =
      require("../src/communities/communityMemberCount").runReconcile;
  });
  ...
  it("skips with reason=pressure when pool queue is at threshold", async () => {
    ...
    expect(
      metricsMock.communityCountPgReconcileSkippedTotal.inc,
    ).toHaveBeenCalledWith({ reason: "pressure" });
  ...
  it("skips with reason=lock when distributed lock is not acquired", async () => {
    ...
    expect(
      metricsMock.communityCountPgReconcileSkippedTotal.inc,
    ).toHaveBeenCalledWith({ reason: "lock" });
    expect(poolMock.query).not.toHaveBeenCalled();
  });
  ...
  it("skips with reason=empty when dirty set has no entries", async () => {
    ...
    expect(
      metricsMock.communityCountPgReconcileSkippedTotal.inc,
    ).toHaveBeenCalledWith({ reason: "empty" });
    expect(poolMock.query).not.toHaveBeenCalled();
  });
  ...
  it("runs batch UPDATE and syncs Redis when dirty entries exist", async () => {
    ...
    expect(redisMock.srem).toHaveBeenCalledWith(
      "community:counts:dirty",
      "c1",
      "c2",
    );
    expect(metricsMock.communityCountPgReconcileTotal.inc).toHaveBeenCalledWith(
      { result: "ok" },
    );
  });
...
describe("GET /communities — member_count Redis overlay", () => {
  ...
  it("returns Redis member_count when Redis has a value", async () => {
    poolMock.queryRead.mockResolvedValue({
      rows: [
        { id: COMMUNITY_ID, name: "test", member_count: 1, is_public: true },
      ],
    });
  ...
  it("falls back to DB member_count when Redis has no entry", async () => {
    poolMock.queryRead.mockResolvedValue({
      rows: [
        { id: COMMUNITY_ID, name: "test", member_count: 7, is_public: true },
      ],
    });
  ...
  it("falls back to DB member_count when Redis throws", async () => {
    poolMock.queryRead.mockResolvedValue({
      rows: [
        { id: COMMUNITY_ID, name: "test", member_count: 9, is_public: true },
      ],
    });
    redisMock.hmget.mockRejectedValue(new Error("ECONNREFUSED"));
