...
**Managed Redis (prod):** App `REDIS_URL` points at **`10.0.1.233:6379`** (private Linode VLAN). Host **`total_system_memory`** is ~**7.75 GiB**; **`maxmemory`** was historically capped at **5 GiB** (leaving headroom unused while **`volatile-lru`** evicted heavily). As of **2026-04-27**, **`CONFIG SET maxmemory`** was raised to **6 GiB** on the instance to reduce eviction churn; re-check after host resize.
