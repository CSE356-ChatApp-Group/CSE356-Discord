...
run_ws_ssh_preflight() {
  ...
  for label in WSVM1 WSVM2 WSVM3; do
    ...
    if ! ssh_vm "${host}" "echo ok >/dev/null"; then
      echo "ERROR: unable to SSH to ${label} (${user}@${host}) with the current deploy key." >&2
      echo "       The dedicated websocket tier is enabled, so this deploy must be able to reach every WS host." >&2
      echo "       Confirm the CI deploy key is present in ${user}@${host}:~/.ssh/authorized_keys." >&2
      exit 1
    fi
  done
