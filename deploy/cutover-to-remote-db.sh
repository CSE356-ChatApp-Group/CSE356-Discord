...
echo "  - On the DB VM: raise max_connections headroom — DB_SSH=${DB_SSH} ./deploy/tune-remote-db-postgres.sh"
echo "    (then ALLOW_DB_RESTART=true if the script says Postgres needs a restart)"
