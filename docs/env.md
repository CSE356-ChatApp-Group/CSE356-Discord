...
| `MEILI_CANDIDATE_LIMIT`, `MEILI_CANDIDATE_MIN_LIMIT`, `MEILI_TIMEOUT_MS`, `MEILI_WRITE_BATCH_SIZE`, `MEILI_WRITE_FLUSH_MS` | Meili HTTP client tuning: candidate limit is a ceiling (default **200**, floor **50** in code; prod can keep **1000** for deep/access-filtered pages while default pages request a smaller overfetch window), candidate minimum default **100**, request timeout (default **2000** ms, clamp **500–5000**), write batch size (**64**), write flush interval (**50** ms). |
