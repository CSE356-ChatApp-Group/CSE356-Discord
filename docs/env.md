...
| `WS_AUTO_SUBSCRIBE_MODE` | `messages` (default) subscribes **`channel:`** + **`conversation:`** + **`user:<self>`** during connect; `user_only` keeps **`user:<self>`** plus sharded community-feed membership so public-channel notifications still arrive without per-channel subscriptions; `full` also eager-subscribes accessible **`community:`** topics. |
