...
queries = {
    ...
    "auth_bcrypt_active_peak": 'max_over_time(max(auth_bcrypt_active{job="chatapp-api"})[12m:30s])',
    "auth_bcrypt_waiters_peak": 'max_over_time(max(auth_bcrypt_waiters{job="chatapp-api"})[12m:30s])',
    "auth_bcrypt_rejects_increase_15m": 'sum by (reason) (increase(auth_bcrypt_queue_rejects_total{job="chatapp-api"}[15m]))',
