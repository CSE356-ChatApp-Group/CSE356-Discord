...
| `MEILI_WRITE_STREAM_CONSUMER_SLOTS`, `MEILI_WRITE_STREAM_READ_COUNT`, `MEILI_WRITE_STREAM_BLOCK_MS`, `MEILI_WRITE_STREAM_COALESCE_MS`, `MEILI_WRITE_STREAM_COALESCE_BLOCK_MS`, `MEILI_WRITE_STREAM_LOCK_TTL_MS`, `MEILI_WRITE_STREAM_CLAIM_IDLE_MS` | Controls the leased Meili stream consumers. Shared prod env can enable consumers on every worker; Redis locks allow only `CONSUMER_SLOTS` active drainers. Defaults: **1** slot, read count = `MEILI_WRITE_BATCH_SIZE`, **1000 ms** initial block, coalesce window = `MEILI_WRITE_FLUSH_MS` clamped to **0–60000 ms**, **100 ms** coalesce poll block, **15000 ms** lock TTL, **60000 ms** pending reclaim idle. |
