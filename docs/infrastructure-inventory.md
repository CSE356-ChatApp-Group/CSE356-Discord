...
**Latency / 5xx bottleneck (typical):** end-user and grader slowness under load most often traces to **PostgreSQL** (long queries, lock contention) and **per-Node connection pools** to PgBouncer/Postgres (`query timeout`, `pg_pool_waiting`, pool circuit breaker / **503**). Under grader bursts, **Redis** (fanout **`PUBLISH`**, **`ws:recent_connect` MGET**, memory at **`maxmemory`**) can also drive **`POST_INSERT_REDIS_TIMEOUT`** and tail latency — confirm with Redis `INFO` / `SLOWLOG` and app logs, not only Postgres.
