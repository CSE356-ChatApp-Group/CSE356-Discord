...
if node "$ROOT_DIR/scripts/load/render-capacity-report.mjs" "$RUN_DIR" "$K6_EXIT" > "$RUN_DIR/report.md"; then
...
if [[ "$PROFILE" == "slo" ]]; then
  ...
  if ! node "$ROOT_DIR/scripts/load/validate-capacity-gates.mjs" "$RUN_DIR" "$PROFILE"; then
    K6_EXIT=1
  fi
fi
