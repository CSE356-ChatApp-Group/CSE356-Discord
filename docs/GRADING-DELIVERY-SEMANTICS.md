...
For **public and private channels**, the server publishes **`message:created`** to **`channel:<id>`** first (unless `CHANNEL_MESSAGE_PUBLISH_CHANNEL_FIRST=false`). Unless `CHANNEL_MESSAGE_USER_FANOUT=0`, it also duplicates the event to **`user:<uuid>`** topics by default ([`backend/src/messages/channelRealtimeFanout.ts`](../backend/src/messages/channelRealtimeFanout.ts)). The safe default is **`CHANNEL_MESSAGE_USER_FANOUT_MODE=all`**, which preserves legacy duplicate fanout to every visible member, still capped by **`CHANNEL_MESSAGE_USER_FANOUT_MAX`**. **`CHANNEL_MESSAGE_USER_FANOUT_MODE=recent_connect`** is an explicit throughput tradeoff for controlled hosts: it limits user-topic duplicates to listeners that connected very recently and assumes stable sockets will receive the message from **`channel:<id>`** after bootstrap. Every connected client subscribes to **`user:<self>`** before the longer **per-channel bootstrap** finishes ([`backend/src/websocket/server.ts`](../backend/src/websocket/server.ts)), so throughput probes that open a WebSocket and POST before all `channel:` subscriptions complete can still receive the event on the **user** topic during that recent-connect window. If a client explicitly **`unsubscribe`s** a `channel:<id>`, duplicate **`user:`** delivery for that channel’s **`message:*`** is suppressed for that socket so opt-out semantics stay correct. The web UI merges duplicates by **message id**; harness clients should do the same.
