...
| `MEILI_CANDIDATE_LIMIT`, `MEILI_TIMEOUT_MS`, `MEILI_WRITE_BATCH_SIZE`, `MEILI_WRITE_FLUSH_MS` | Meili HTTP client tuning: candidate limit (default **200**, floor **50** in code; prod set to **1000** to reduce strict-token-mismatch fallbacks), request timeout (default **2000** ms, clamp **500–5000**), write batch size (**64**), write flush interval (**50** ms). |
