...
ssh -o BatchMode=yes -o ConnectTimeout=15 "${PROD_USER}@${PROD_HOST}" "PROD_HOST='${PROD_HOST}' PRIMARY_VM1_HOST='${PRIMARY_VM1_HOST}' WSVM1_INTERNAL='${WSVM1_INTERNAL}' WSVM2_INTERNAL='${WSVM2_INTERNAL}' WSVM3_INTERNAL='${WSVM3_INTERNAL}' WSVM1_WORKERS='${WSVM1_WORKERS}' WSVM2_WORKERS='${WSVM2_WORKERS}' WSVM3_WORKERS='${WSVM3_WORKERS}' WS_TIER_ENABLED='${WS_TIER_ENABLED}' bash" <<'REMOTE'
set -euo pipefail
...
if [[ "${PROD_HOST}" == "${PRIMARY_VM1_HOST}" ]]; then
  ...
  if [[ -n "${WS_UPSTREAM}" ]]; then
    ...
    if [[ "${WS_TIER_ENABLED:-false}" == "true" ]]; then
      ...
      if ! grep -Fq 'proxy_pass http://app;' <<< "${WS_LOCATION}"; then
        echo "FAIL: /ws should route to upstream app while websocket tier is disabled"
        exit 1
      fi
      echo "OK: /ws routes to shared app upstream while websocket tier is disabled"
    fi
  fi
else
