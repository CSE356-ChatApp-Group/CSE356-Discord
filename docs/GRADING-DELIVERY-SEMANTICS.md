...
For **public and private channels**, the server publishes **`message:created`** to **`channel:<id>`** first (unless `CHANNEL_MESSAGE_PUBLISH_CHANNEL_FIRST=false`). Unless `CHANNEL_MESSAGE_USER_FANOUT=0`, it may also duplicate the event to **`user:<uuid>`** topics ([`backend/src/messages/channelRealtimeFanout.ts`](../backend/src/messages/channelRealtimeFanout.ts)). In the default **`CHANNEL_MESSAGE_USER_FANOUT_MODE=recent_connect`**, those user-topic duplicates are limited to listeners that connected very recently, which preserves the bootstrap bridge without paying a per-member publish cost on every send. Set **`CHANNEL_MESSAGE_USER_FANOUT_MODE=all`** to restore legacy duplicate fanout to every visible member, still capped by **`CHANNEL_MESSAGE_USER_FANOUT_MAX`**. Every connected client subscribes to **`user:<self>`** before the longer **per-channel bootstrap** finishes ([`backend/src/websocket/server.ts`](../backend/src/websocket/server.ts)), so **throughput probes that open a WebSocket and POST before all `channel:` subscriptions complete** can still receive the event on the **user** topic during that recent-connect window. If a client explicitly **`unsubscribe`s** a `channel:<id>`, duplicate **`user:`** delivery for that channel’s **`message:*`** is suppressed for that socket so opt-out semantics stay correct. The web UI merges duplicates by **message id**; harness clients should do the same.
