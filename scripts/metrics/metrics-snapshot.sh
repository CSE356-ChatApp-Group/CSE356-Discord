...
queries=(
  ...
  'histogram_quantile(0.99, sum by (le, path) (rate(fanout_target_candidates_bucket{job="chatapp-api"}[5m])))'
  'histogram_quantile(0.95, sum by (le, path) (rate(ws_active_subscriber_targets_bucket_bucket{job="chatapp-api"}[5m])))'
  'histogram_quantile(0.99, sum by (le, path) (rate(ws_active_subscriber_targets_bucket_bucket{job="chatapp-api"}[5m])))'
  ...
  'histogram_quantile(0.95, sum by (le, topic_prefix, vm, worker) (rate(ws_pubsub_receive_lag_ms_bucket{job="chatapp-api"}[5m])))'
  'sum by (topic_prefix, vm, worker) (rate(ws_pubsub_messages_total{job="chatapp-api"}[5m]))'
  'sum by (topic_prefix, vm, worker) (rate(ws_pubsub_recipient_slots_total{job="chatapp-api"}[5m]))'
  'topk(10, sum by (shard) (rate(ws_userfeed_publish_targets_total{job="chatapp-api"}[5m])))'
  'topk(10, sum by (shard) (rate(ws_userfeed_envelope_users_total{job="chatapp-api"}[5m])))'
  'sum by (vm, worker) (rate(ws_userfeed_local_recipients_total{job="chatapp-api"}[5m]))'
  'max by (vm, worker) (ws_userfeed_owned_shards{job="chatapp-api"})'
