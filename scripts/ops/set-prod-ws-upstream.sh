...
if [[ -z "${WSVM1_INTERNAL}" ]] && [[ -z "${WSVM2_INTERNAL}" ]] && [[ -z "${WSVM3_INTERNAL}" ]]; then
  echo "ERROR: no websocket VM private IPs configured (WSVM1_INTERNAL/WSVM2_INTERNAL/WSVM3_INTERNAL)."
  exit 1
fi

ssh -o BatchMode=yes -o ConnectTimeout=15 "${PROD_USER}@${PROD_HOST}" \
  "WSVM1_INTERNAL='${WSVM1_INTERNAL}' WSVM2_INTERNAL='${WSVM2_INTERNAL}' WSVM3_INTERNAL='${WSVM3_INTERNAL}' WSVM1_WORKERS='${WSVM1_WORKERS}' WSVM2_WORKERS='${WSVM2_WORKERS}' WSVM3_WORKERS='${WSVM3_WORKERS}' bash" <<'REMOTE'
set -euo pipefail
