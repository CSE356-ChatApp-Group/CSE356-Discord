...
#   ./scripts/run-prod-nginx-ladder-metrics.sh
#
...
#
# DB statement hotness (not in this poller): from a host with DB access run scripts/pg-stat-statements-snapshot.sh
...
if [[ -n "${METRICS_SNAPSHOT_RANGE:-}" ]] || [[ "${APPEND_METRICS_SNAPSHOT:-1}" == 1 ]]; then
  METRICS_SNAPSHOT_RANGE="${METRICS_SNAPSHOT_RANGE:-3m}" PROMETHEUS_URL="$PROM" ./scripts/metrics-snapshot.sh >>"$OUT" 2>&1 || true
fi
