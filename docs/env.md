...
| `READ_RECEIPT_FANOUT_ENABLED` | Enables async `read:updated` fanout publishes from `PUT /messages/:id/read`. Default **`false`** for burst resilience; read state remains durable via Redis cursor CAS + batched DB flush. |
