...
#
#   ./scripts/load/run-local-hot-channel-convoy-e2e.sh
#   CONCURRENCY=8 DURATION_SEC=45 ./scripts/load/run-local-hot-channel-convoy-e2e.sh
#
# Skip Docker (use existing API + Prom): provide TOKEN + CHANNEL_ID
#   SKIP_COMPOSE=1 BASE_URL=http://localhost/api/v1 TOKEN=... CHANNEL_ID=... ./scripts/load/run-local-hot-channel-convoy-e2e.sh
#
...
if [[ -z "${TOKEN:-}" || -z "${CHANNEL_ID:-}" ]]; then
  ...
  BASE_URL="${API_BASE}" node "$ROOT/scripts/load/local-api-seed-channel.mjs" >"$SEED_FILE"
...
METRICS_BURST_WINDOW="$METRICS_BURST_WINDOW" \
  ...
  bash "$ROOT/scripts/load/hot-channel-convoy-validate.sh" | tee "${OUT_PREFIX}-prom.json"
