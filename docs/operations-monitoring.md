...
## Redis pubsub hotspot triage

Use this section when websocket delivery is slow **before socket write** (`ws.delivery.slow_trace` dominated by `pubsub_receive_lag_ms`) or when a dedicated WS tier shows worker skew.

### New first-look series

| Question | Series |
|----------|--------|
| Which workers are receiving the most realtime bus traffic? | `sum(rate(ws_pubsub_messages_total{job="chatapp-api"}[5m])) by (topic_prefix, vm, worker)` |
| Which workers see the most local recipients on pubsub receipt? | `sum(rate(ws_pubsub_recipient_slots_total{job="chatapp-api"}[5m])) by (topic_prefix, vm, worker)` |
| Which `userfeed` shards are hottest on publish? | `topk(10, sum(rate(ws_userfeed_publish_targets_total{job="chatapp-api"}[5m])) by (shard))` |
| Which `userfeed` shards are hottest on receive? | `topk(10, sum(rate(ws_userfeed_envelope_users_total{job="chatapp-api"}[5m])) by (shard))` |
| Which workers are doing the most real userfeed delivery work? | `sum(rate(ws_userfeed_local_recipients_total{job="chatapp-api"}[5m])) by (vm, worker)` |
| Are workers only owning a subset of userfeed shards now? | `max(ws_userfeed_owned_shards{job="chatapp-api"}) by (vm, worker)` |
| Is lag on the direct topic path or the shared userfeed path? | `histogram_quantile(0.95, sum by (le, topic_prefix, vm, worker) (rate(ws_pubsub_receive_lag_ms_bucket{job="chatapp-api"}[5m])))` |

### How to read them

- If `ws_userfeed_publish_targets_total` is skewed by `shard`, the **sender-side hash** is concentrating hot users on a few userfeed shards.
- If publish is balanced but `ws_userfeed_local_recipients_total` is skewed by `worker`, the problem is **sticky session / recipient placement** on the websocket workers.
- If `ws_pubsub_messages_total{topic_prefix="userfeed"}` is high on every worker but `ws_userfeed_local_recipients_total` is low on most of them, the current shared `userfeed` bus is wasting per-worker JSON parse / event-loop work and a subscriber-ownership partition is the next architecture lever.
- If `ws_pubsub_receive_lag_ms` is high for `channel` / `conversation` as well as `userfeed`, suspect the Redis pubsub bus or the receiving worker event loop more broadly, not just the recent-connect/userfeed bridge.
...
| Realtime | `redis_fanout_publish_failures_total`, `fanout_publish_duration_ms`, `fanout_publish_targets`, `fanout_target_candidates`, `ws_active_subscriber_targets_bucket`, `ws_fanout_offline_skipped_total`, `redis_exists_by_path_total`, `ws_recipient_duplicate_candidates_total`, `fanout_target_cache_total`, `conversation_fanout_targets_cache_version_retry_total`, `ws_bootstrap_wall_duration_ms`, `ws_bootstrap_channels`, `ws_bootstrap_list_cache_total`, `ws_backpressure_events_total`, `ws_reliable_delivery_total`, `ws_reliable_delivery_latency_ms`, `channel_message_fanout_recipient_total`, `realtime_miss_attribution_total`, `pending_replay_recipient_total`, `pending_replay_entries_per_message`, `offline_pending_skipped_total`, `ws_pending_user_zset_size`, `redis_lua_script_load_total`, `redis_lua_eval_total`, `redis_lua_noscript_retry_total`, `ws_pubsub_receive_lag_ms`, `ws_pubsub_messages_total`, `ws_pubsub_recipient_slots_total`, `ws_userfeed_publish_targets_total`, `ws_userfeed_envelope_users_total`, `ws_userfeed_local_recipients_total` | Fanout health, Redis publish multiplier, active subscriber targets, offline skips before realtime work, Redis `EXISTS` probe rate, duplicate candidates after cross-path dedupe, target-cache effectiveness, conversation fanout cache invalidation races, WS bootstrap breadth, slow clients, Redis Lua cache churn, and pubsub hotspot detection. **`ws_reliable_delivery_total{path,source}`** counts each reliable event actually **`ws.send`** after dedupe: **`path=realtime`** (`source=live_pubsub`) vs **`path=replay`** (`source=missed_db` reconnect SQL backfill vs `pending_queue` Redis pending drain). **`ws_reliable_delivery_latency_ms`** is ms from **`created_at` / `publishedAt`** to send (same path label). **`channel_message_fanout_recipient_total{segment}`** splits channel **`message:created`** user-topic work: **`candidate`**, **`inline_user_topic`**, **`deferred_user_topic`** (deferred = not in recent-connect inline set when HTTP blocking is off). **`ws_pubsub_*`** and **`ws_userfeed_*`** are the fastest way to distinguish sender-side shard skew from receiver-side worker hotspotting. |
