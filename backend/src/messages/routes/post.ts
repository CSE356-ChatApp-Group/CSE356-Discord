  ...
  router.post(
    ...
    async (req, res, next) => {
      ...
      try {
            ...
            logger.warn(
              {
                requestId: authReq.id,
                reason,
                target: authReq.body.channelId ? "channel" : "conversation",
              },
        ...
        if (err?.code === "23503") {
          logger.warn(
            { requestId: authReq.id, constraint: err.constraint, detail: err.detail },
        ...
        if (isMessagePostInsertDbTimeout(err)) {
          ...
          return res
            .status(503)
            .set("Retry-After", "1")
            .json(messagePostBusy503Body(authReq, "message_post_insert_timeout"));
        }
        if (isChannelInsertLockTimeoutError(err)) {
          ...
          return res
            ...
            .set("Retry-After", "1")
            .json(
              messagePostBusy503Body(authReq, lockApiCode, {
        ...
        if (isChannelInsertLockQueueRejectError(err)) {
          ...
          return res
            ...
            .set("Retry-After", "1")
            .json(
              messagePostBusy503Body(authReq, "message_insert_lock_waiter_cap", {
