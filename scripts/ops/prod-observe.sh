...
#   PROD_USER=ubuntu PROD_HOST=130.245.136.44 SINCE='4 hours ago' ./scripts/ops/prod-observe.sh
#   PROD_PUBLIC_HOST=group-8.cse356.compas.cs.stonybrook.edu ./scripts/ops/prod-observe.sh
#   POST_MSG_MINUTES=15 ./scripts/ops/prod-observe.sh   # rolling window for POST /messages breakdown
# Repeat checks every 2m (only POST /messages block + header):  watch -n 120 'POST_MSG_MINUTES=30 ./scripts/ops/prod-observe.sh 2>&1 | head -n 12'
set -euo pipefail
