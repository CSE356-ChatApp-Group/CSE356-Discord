...
describe('presence fanout_targets cache invalidation', () => {
  ...
  it('bulk: empty input performs no Redis unlink/del', async () => {
    ...
    expect(metrics.presenceFanoutTargetsInvalidationKeysTotal.inc).not.toHaveBeenCalled();
  });
  ...
  it('bulk: duplicate userIds are de-duped to one unlink call', async () => {
    ...
    expect(metrics.presenceFanoutTargetsInvalidationKeysTotal.inc).toHaveBeenCalledWith(
      { mode: 'bulk' },
      1,
    );
  });
  ...
  it('bulk: <=512 unique keys triggers one unlink', async () => {
    ...
    expect(metrics.presenceFanoutTargetsInvalidationTotal.inc).toHaveBeenCalledWith(
      { mode: 'bulk', command: 'unlink' },
      1,
    );
  });
  ...
  it('bulk: >512 keys triggers multiple unlink chunks', async () => {
    ...
    expect(metrics.presenceFanoutTargetsInvalidationTotal.inc).toHaveBeenCalledTimes(2);
    expect(metrics.presenceFanoutTargetsInvalidationKeysTotal.inc).toHaveBeenCalledWith(
      { mode: 'bulk' },
      600,
    );
  ...
  it('bulk: falls back to del when unlink is unavailable', async () => {
    ...
    expect(metrics.presenceFanoutTargetsInvalidationTotal.inc).toHaveBeenCalledWith(
      { mode: 'bulk', command: 'del_fallback' },
      1,
    );
  ...
  it('setPresence: corrupt fanout cache triggers single-key unlink before DB reload', async () => {
    ...
    expect(redis.unlink).toHaveBeenCalledWith(`presence:${actor}:fanout_targets`);
