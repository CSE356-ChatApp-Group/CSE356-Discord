...
| `WS_BOOTSTRAP_STORM_PROTECTION_ENABLED` | Enable rate-limited progressive subscription hydration to prevent reconnect storms from blocking live delivery. Default **true**. |
| `WS_BOOTSTRAP_HYDRATION_MAX_CONCURRENT` | Max concurrent bootstrap hydration jobs per worker. Default **8**. |
| `WS_BOOTSTRAP_HYDRATION_BATCH_INTERVAL_MS` | Polling interval when hydration queue is at concurrency cap. Default **50** ms. |
| `WS_BOOTSTRAP_HYDRATION_JITTER_MAX_MS` | Max random jitter between hydration starts. Default **100** ms. |
| `WS_BOOTSTRAP_COALESCE_WINDOW_MS` | Window to coalesce duplicate hydration for the same user. Default **5000** ms. |
| `WS_BOOTSTRAP_LIVE_FANOUT_YIELD_MS` | Sleep duration when yielding to live fanout. Default **20** ms. |
| `WS_FANOUT_RECIPIENT_DEDUPE_TTL_MS` | TTL for per-message, per-recipient deduplication across fanout paths. Default **30000** ms. |
| `WS_FANOUT_RECIPIENT_DEDUPE_MAX_ENTRIES` | Max entries in the fanout recipient dedupe map. Default **100000**. |
