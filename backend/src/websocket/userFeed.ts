...
const USER_FEED_SHARD_COUNT =
  Number.isFinite(rawUserFeedShardCount) && rawUserFeedShardCount > 0
    ...
    : 64;
const rawLiveOwnerResolutionMax = Number(process.env.USER_FEED_LIVE_OWNER_RESOLUTION_MAX || '64');
const USER_FEED_LIVE_OWNER_RESOLUTION_MAX =
  Number.isFinite(rawLiveOwnerResolutionMax) && rawLiveOwnerResolutionMax >= 0
    ? Math.max(0, Math.min(512, Math.floor(rawLiveOwnerResolutionMax)))
    : 64;
...
async function resolveLiveWorkerOwnedUsers(userIds) {
  ...
  if (
    !connectionPipe
    || typeof connectionPipe.smembers !== 'function'
  ) {
    return resolveWorkerOwnedUsers(userIds);
  }
  for (const userId of userIds) {
  ...
  if (
    !ownerPipe
    || typeof ownerPipe.exists !== 'function'
    || typeof ownerPipe.get !== 'function'
  ) {
    return resolveWorkerOwnedUsers(userIds);
  }
  const ownerLookups = [];
...
async function publishUserFeedTargets(userTargets, payload, { preferLiveOwners = false } = {}) {
  ...
  const useLiveOwners =
    preferLiveOwners
    || (USER_FEED_LIVE_OWNER_RESOLUTION_MAX > 0 && userIds.length <= USER_FEED_LIVE_OWNER_RESOLUTION_MAX);

  const workerOwned = useLiveOwners
