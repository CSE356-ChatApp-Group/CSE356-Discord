...
| `READ_RECEIPT_SHED_ON_MESSAGE_INSERT_TIMEOUT_ENABLED` | When **`true`** (default), a recent **`POST /api/v1/messages`** insert-phase statement/query timeout (`57014` / `message_post_insert_timeout`) extends a process-local window where **`PUT /messages/:id/read`** returns **`200` + `deferred: true`** (`reason: message_insert_unhealthy`) without Redis CAS / DB work — protects inserts on the same worker. Set **`false`** / **`0`** to disable. |
| `READ_RECEIPT_MESSAGE_INSERT_UNHEALTHY_TTL_MS` | How long (ms) each qualifying POST insert-timeout extends the read-defer window on that worker (default **10000**, clamp **1000–120000**). Later timeouts refresh the deadline (`max(now, existingUntil)`). |
