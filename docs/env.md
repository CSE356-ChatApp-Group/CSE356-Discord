...
**Slow non-message HTTP trace (app workers, optional):** Set **`SLOW_HTTP_TRACE_MIN_MS`** (e.g. **`2000`**) to emit **`slow_http_request_trace`** for requests whose total wall time exceeds the threshold, with **`route`**, **`requestId`**, **`worker_id`**, **`db_query_count`**, **`db_sum_ms`**, **`db_max_single_ms`**, per-query **`db_query_samples`** (truncated SQL + ms), and **`app_wall_ms_estimated`** (residual vs summed DB time; see **`db_wall_parallel_overlap_hint`** when `Promise.all` overlaps). Default **`SLOW_HTTP_TRACE_EXCLUDE_PREFIXES`** is **`/api/v1/messages,/health,/metrics`** so **`POST /messages`** stays on the dedicated message trace. For targeted canaries, set **`SLOW_HTTP_TRACE_INCLUDE_ROUTES`** (comma-separated exact route/raw path values, e.g. **`/api/v1/messages/:id/read`**) to bypass excludes for just those routes. See [`docs/operations-monitoring.md#slow-route-explain-workflow`](operations-monitoring.md#slow-route-explain-workflow).
...
## Realtime baseline profile (recommended)
...
6. Set **`WS_PROFILE=generated-client`** so these defaults stay coherent if individual env vars are omitted.

Treat other websocket env vars as advanced/incident knobs unless a benchmark shows this baseline is insufficient.
...
**Repository audit (no server access):** [`docker-compose.yml`](../docker-compose.yml) sets high register/login limits (500) **only** for the local `api` service to support parallel E2E; production does not use that compose stack as-is. Deploy scripts (`deploy/deploy-staging.sh`, `deploy/deploy-prod.sh`) tee shell defaults, then `apply-env-profile.py` merges `deploy/env/staging.required.env` or `deploy/env/prod.required.env`; treat those profile files as the source of truth for pinned production/staging values.
...
| `SLOW_HTTP_TRACE_MIN_MS`, `SLOW_HTTP_TRACE_EXCLUDE_PREFIXES`, `SLOW_HTTP_TRACE_INCLUDE_ROUTES` | Optional slow-request structured tracing for non-`POST /messages` paths. Excludes default to `/api/v1/messages,/health,/metrics`; include routes (exact route/raw path) override excludes for targeted canaries like `/api/v1/messages/:id/read`. |
...
| `READ_RECEIPT_SHED_ON_MESSAGE_INSERT_TIMEOUT_ENABLED` | When **`true`** (default), a **`POST /api/v1/messages`** insert-phase statement/query timeout (`57014` / `message_post_insert_timeout`) triggers read-receipt shedding: process-local defer on that worker **plus** optional Redis **`SET health:message_insert_unhealthy`** so **other** workers can **`GET`** it on a background poll (no Redis read per request). **`PUT /messages/:id/read`** returns **`200` + `deferred: true`** (`reason: message_insert_unhealthy`) without Redis CAS / DB work when unhealthy. Set **`false`** / **`0`** to disable **both** local and fleet-visible paths (no Redis **`SET`**, no poll). |
| `READ_RECEIPT_MESSAGE_INSERT_UNHEALTHY_TTL_MS` | How long (ms) each qualifying POST insert-timeout extends the process-local read-defer window and Redis **`PX`** TTL on **`health:message_insert_unhealthy`** (default **10000**, clamp **1000–120000**). Later timeouts refresh the deadline (`max(now, existingUntil)`). |
| `READ_RECEIPT_GLOBAL_INSERT_UNHEALTHY_POLL_MS` | Interval (ms) between background **`GET`** checks for **`health:message_insert_unhealthy`** on each worker when shedding is enabled (default **500**, clamp **250–1000**). Preflight itself stays synchronous and only reads cached memory updated by this poller. |
...
| `READ_RECEIPT_MESSAGE_ACK_CACHE_ENABLED` | When **`true`/`1`/`yes`**, **`PUT /messages/:id/read`** checks Redis **`read_receipt_msg_ack:{userId}:{messageId}`** after local no-op checks and before **`loadMessageTargetForUser`**: if present, returns **`200` + `{ success: true }`** without target lookup, CAS Lua, fanout, or PG enqueue. **Default (unset): off** — opt in after validation; no API contract change. Fail-open on Redis errors. See [`readReceiptMessageAckCache.ts`](../backend/src/messages/readReceipt/readReceiptMessageAckCache.ts). |
| `READ_RECEIPT_MESSAGE_ACK_CACHE_TTL_MS` | **`PX`** TTL for **`read_receipt_msg_ack:*`** markers (default **60000**, clamp **5000–600000**). Keys are set only after access is confirmed and the read is accepted (not on 403/404/preflight deferrals). |
| `READ_RECEIPT_DROP_FANOUT_ON_WS_PRESSURE_ENABLED` | When enabled, **`PUT /messages/:id/read`** still advances durable read state but skips lower-priority realtime read-receipt fanout while WebSocket delivery pressure is active. Default **true** in code; set **`false`** / **`0`** to disable. |
| `READ_RECEIPT_FULL_DEFER_ON_WS_PRESSURE_ENABLED` | Emergency gate for active delivery failure periods. When **`true`** and WebSocket delivery pressure is active, **`PUT /messages/:id/read`** returns **`200` + `{ success: true, deferred: true, reason: "ws_delivery_pressure" }`** at read preflight, before target lookup, cursor CAS/read-state enqueue, ack-cache success recording, or read-receipt fanout. Default **false**. |
| `READ_RECEIPT_WS_PRESSURE_TTL_MS` | How long a WebSocket pressure signal suppresses read-receipt fanout (default **30000**, clamp **5000–300000**). |
| `READ_RECEIPT_WS_PRESSURE_REALTIME_LATENCY_MS` | Marks WebSocket delivery pressure when an observed realtime reliable-delivery latency sample reaches this threshold (default **5000**, clamp **1000–300000**). |
| `READ_RECEIPT_WS_PRESSURE_BOOTSTRAP_WALL_MS` | Marks WebSocket delivery pressure when a socket bootstrap wall-time sample reaches this threshold (default **10000**, clamp **1000–300000**). |
| `READ_RECEIPT_WS_PRESSURE_MISS_COUNT` | Marks WebSocket delivery pressure when process-local realtime miss attribution reaches this count in a short rolling window for relevant delivery-loss reasons (default **20**). |
...
| `CHANNEL_MESSAGE_SKIP_USERFEED_PUBLISH` | When **`true`**, channel `message:created` skips duplicate userfeed publishes for active/recent users that have already hydrated the direct `channel:<id>` subscription. During progressive bootstrap and join/invite subscribe pushes, the server marks `channel:bootstrap_pending:<channelId>` and `subscribeClient` clears the marker after the socket joins `channel:<id>`, so the userfeed bridge remains active only for recipients still in that gap. The bootstrap-pending bridge is capped by `CHANNEL_MESSAGE_USER_FANOUT_MAX`, not the narrower immediate recent-connect cap, so large-channel reconnect storms do not strand connected-but-not-yet-hydrated clients. Production pins **`true`** for Redis pub/sub savings; set **`false`** to restore the broader recent-connect duplicate bridge. |
| `CHANNEL_BOOTSTRAP_PENDING_TTL_SECONDS` | TTL for `channel:bootstrap_pending:<channelId>` markers (default follows the old short bootstrap window: `min(WS_RECENT_CONNECT_TTL_SECONDS, 60)`, clamped **30-600**). Production pins **300** so a dropped internal subscribe after community join/private invite does not strand an already-open generated client when `WS_RECENT_CONNECT_TTL_SECONDS=120` expires. Markers are removed as soon as `subscribeClient` confirms `channel:<id>` subscription. |
...
| `RECENT_CONNECT_TARGET_CACHE_MS` | In-process cache TTL (ms) for “recent connect” fanout target resolution (`channelRealtimeFanout.ts`; default **1500**). Staged/prod pin **250** to shorten stale-target windows after reconnect/bootstrap. |
| `CHANNEL_MESSAGE_RECENT_CONNECT_FALLBACK_PROBE_MAX` | Max non-ZSET recipients to probe via `ws:recent_connect:*`/connected-user fallback per channel message when `CHANNEL_MESSAGE_USER_FANOUT_MODE=recent_connect` (default **512**, clamp **64–5000**). Keeps reconnect bridge coverage while bounding Redis work on very large channels. |
| `CHANNEL_MESSAGE_RECENT_CONNECT_INCLUDE_CONNECTED_FALLBACK` | Controls whether `recent_connect` fanout also probes active connected-user membership (`presence:connected_users`) for non-ZSET recipients. Defaults to **`false`** when `WS_AUTO_SUBSCRIBE_MODE=messages`, else **`true`**. Set explicitly to force behavior. |
| `CONVERSATION_FANOUT_TARGETS_CACHE_TTL_SECS` | Redis TTL for cached conversation participant fanout audiences used by DM/group-DM realtime publishes (default `120`) |
| `CONVERSATION_FANOUT_TARGETS_CACHE_WARMUP_MISSES`, `CONVERSATION_FANOUT_TARGETS_CACHE_WARMUP_WINDOW_MS` | Avoid low-value one-off writes for conversation fanout target cache: require N misses within window before writing Redis entry (defaults **2** misses in **30000** ms). Reduces churn when conversation IDs have low short-term reuse. |
...
| `MESSAGE_POST_FAST_ACCEPT_ENABLED` | When enabled (**default true**), successful `POST /messages` uses the insert-returned row for the HTTP body when no attachment hydration is needed, avoiding an extra post-insert Postgres read on the hot path. |
| `MESSAGE_POST_AWAIT_CACHE_BUST` | When **false**, `POST /messages` starts message-list cache invalidation but does not wait for it before **201**. Default **true** for read-your-writes compatibility; prod can pin **false** when websocket/replay paths carry freshness. |
...
| `MESSAGE_POST_SYNC_FANOUT` | Set **`1`/`true`** to restore **pre-async** behavior: **`await`** **`publishChannelMessageCreated` / `publishConversationEventNow`** on the request thread. This is safer for strict immediate delivery semantics but can add multi-second POST tail latency under Redis/fanout pressure. Default (unset or explicit `false`) keeps async enqueue via **`messagePostFanoutAsync`** (dedupe, retries, dead-letter). Current prod baseline pins **`false`**; enable only for short diagnostic windows. |
...
| | Read-receipt defaults (`READ_RECEIPT_*`, `READ_CURSOR_*`, …) are frozen at startup in [`readReceiptConfig.ts`](../backend/src/messages/config/readReceiptConfig.ts). |
...
| `READ_RECEIPT_INVALIDATE_CHANNELS_LIST_CACHE` | **Ignored (always off):** `channels:list` deletes on read receipts were removed from `readReceiptHttpCore.ts` to stop structural list cache churn. REST **`GET /channels`** may lag unread until **`CHANNELS_LIST_CACHE_TTL_SECS`** for clients without WS; **`read:updated`** and Redis watermark keys still drive live UI. |
...
| `LAST_MESSAGE_PG_RECONCILE_ENABLED` | `true` to enable background DB reconcile of `channels.last_message_*` from Redis metadata **and** delete-time `repointChannelLastMessage` DB updates; default `false` keeps channel latest-message metadata Redis-first with DB as stale fallback. **Staging/prod:** pinned `false` in [`deploy/env/staging.required.env`](../deploy/env/staging.required.env) / [`deploy/env/prod.required.env`](../deploy/env/prod.required.env); deploy applies via [`deploy/apply-env-profile.py`](../deploy/apply-env-profile.py). |
...
| `CONVERSATION_LAST_MESSAGE_PG_RECONCILE_ENABLED` | `true` to enable background DB reconcile of `conversations.last_message_*` from Redis metadata **and** delete-time `repointConversationLastMessage` DB updates; default `false` keeps conversation latest-message metadata Redis-first with DB as stale fallback (same deploy pin as `LAST_MESSAGE_PG_RECONCILE_ENABLED`) |
...
| `PG_READ_REPLICA_URL`, `PG_READ_POOL_MAX` | Optional read replica for `GET /api/v1/messages` list `SELECT`s ([`docs/architecture/db-scaling-messages.md`](architecture/db-scaling-messages.md)). Request **`X-ChatApp-Read-Consistency: primary`** on that GET to force the primary when you need read-your-writes after a POST. |
...
| `PRESENCE_FANOUT_RECIPIENT_LOOKUP_MODE` | Presence update audience source: `db_cache` (default) uses cached DB-derived community/DM recipients; `cache_only`/`redis_only` uses the cache if present and otherwise self; `self` skips the expensive DB recipient self-join entirely. |
...
| `PRESENCE_DB_MIRROR_MODE`, `PRESENCE_DB_MIRROR_FLUSH_INTERVAL_MS`, `PRESENCE_DB_MIRROR_BATCH_SIZE` | Presence snapshot mirror mode: `async` (default) coalesces Redis-first status changes and batch upserts Postgres in the background; `inline` restores the old per-change DB upsert; `off` disables the analytics mirror. |
...
| `REALTIME_EVENT_ALIAS_FANOUT` | When **`1`/`true`/`yes`**, `fanout.publish` / `fanout.publishBatch` (including sharded userfeed batches) also publishes **alias** event names on the same Redis channel — for example `new_message` beside `message:created`, `message:edited` / `message_edited` beside `message:updated`, and read-receipt aliases beside `read:updated`. Default **off**. **Not required** for the in-repo grader `GeneratedClient` (see [`docs/architecture/websocket-generated-client-parity.md`](architecture/websocket-generated-client-parity.md)); enable only for third-party harnesses that insist on alternate names. When on, permissive clients may invoke handlers twice. **Canonical + alias tables and WS “reliable event” classification** live in [`backend/src/realtime/realtimeEventAliases.ts`](../backend/src/realtime/realtimeEventAliases.ts) (one file to edit when adding names). |
| `WS_PROFILE` | Optional websocket behavior profile. **`generated-client`** sets coherent defaults for this repo’s generated client assumptions (for example: `WS_AUTO_SUBSCRIBE_MODE=messages`, `CHANNEL_MESSAGE_USER_FANOUT_MODE=recent_connect`, and reduced recent-connect fallback probing). Production/staging still override individual knobs explicitly when reconnect/delivery audits require a wider bridge (for example enabling connected-user fallback while keeping `recent_connect`). Explicit env vars still override profile defaults. |
...
| `WS_BOOTSTRAP_PROGRESSIVE_READY` | Feature-gated progressive ready path. Default **false**. When enabled, the server sends `ready` after auth, `user:<id>` subscribe, reconnect replay/pending drain, bootstrap list load, ACL warm, and recent-connect priming; full channel/conversation/community subscription hydration continues in the background and emits `bootstrap:complete` when finished. Ready flags are honest: `bootstrapComplete:false`, `subscriptionsHydrated:false`, `progressiveHydration:true`. Production pins **false** for generated-client grading because that client treats `ready` as delivery-ready and does not wait for `bootstrap:complete`. |
| `WS_REPLAY_SKIP_DB_WHEN_PENDING_HIT` | When **true**, reconnect first drains Redis pending replay and skips the heavier DB replay if pending delivered at least one message. Default **false**; prod can enable when `WS_REPLAY_PENDING_ONLY_ACTIVE` is healthy. |
| `WS_ACL_CACHE_MAX_ENTRIES`, `WS_ACL_REDIS_TTL_SECS`, `WS_BOOTSTRAP_BATCH_SIZE`, `WS_BOOTSTRAP_CACHE_TTL_SECONDS`, `WS_BOOTSTRAP_INGRESS_TTL_SECONDS`, `WS_BOOTSTRAP_INGRESS_JITTER_MAX_MS`, `WS_BOOTSTRAP_DB_MAX_IN_FLIGHT`, `WS_BOOTSTRAP_DB_CONCURRENCY_WAIT_MS`, `WS_RECENT_CONNECT_TTL_SECONDS`, `WS_RECENT_DISCONNECT_TTL_SECONDS` | WS ACL cache + bootstrap tuning (defaults: ACL Redis TTL derived from ACL cache window, bootstrap batch **96** in code vs **64** in deploy profiles, ingress TTL **5** s with small write-time jitter to avoid synchronized expirations, ingress DB-start jitter **200** ms, DB in-flight **50**, DB wait **300** ms, recent-connect **20** s in code / **300** s in deploy, disconnect marker TTL **3600** s). |
| `WS_AUTO_SUBSCRIBE_MODE` | `messages` subscribes **`channel:`** + **`conversation:`** + **`user:<self>`** during connect; `user_only` keeps **`user:<self>`** plus sharded community-feed membership so public-channel notifications still arrive without per-channel subscriptions. Current staged/prod baseline pins **`messages`** for generated clients that do not explicitly send subscribe frames after `ready`. `full` also eager-subscribes accessible **`community:`** topics. |
...
| `RUNTIME_LOG_CONTROL_ENABLED` | When **`true`**, high-cost log categories can be force-enabled/disabled at runtime via Redis keys (no deploy/restart). Default **`false`**. |
| `RUNTIME_LOG_CONTROL_KEY_PREFIX`, `RUNTIME_LOG_CONTROL_REFRESH_MS` | Runtime log-control Redis prefix (default **`logctl`**) and local refresh interval ms (default **5000**, min **1000**). |
| `logctl:slow_http_request_trace`, `logctl:post_messages_e2e_trace`, `logctl:ws_hot_info` | Redis runtime category keys (value **`1`/`true`/`on`** = force enabled, **`0`/`false`/`off`** = force disabled, missing = use env/default behavior). Example: `redis-cli -u \"$REDIS_URL\" SET logctl:slow_http_request_trace 0`. |
...
| **Search (Meilisearch)** | |
| `SEARCH_BACKEND` | Candidate source selector: **`meili`** (with **`MEILI_ENABLED=true`**) or **`opensearch`** (POC-only with `OPENSEARCH_READ_ENABLED=true`). Postgres remains final recheck/response authority. |
...
| `MEILI_CANDIDATE_LIMIT`, `MEILI_CANDIDATE_MIN_LIMIT`, `MEILI_TIMEOUT_MS`, `MEILI_HTTP_KEEPALIVE_ENABLED`, `MEILI_WRITE_BATCH_SIZE`, `MEILI_WRITE_FLUSH_MS` | Meili HTTP client tuning: candidate limit is a ceiling (default **200**, floor **50** in code; prod can keep **1000** for deep/access-filtered pages while default pages request a smaller overfetch window), candidate minimum default **100**, request timeout (default **2000** ms, clamp **500–5000**), persistent HTTP keep-alive is enabled by default to avoid private-network TCP handshake tails, write batch size (**64**, prod **200**) — caps documents per single Meili documents POST on **both** the local in-process flush and the stream consumer (the stream consumer chunks coalesced batches by `MEILI_WRITE_STREAM_TASK_CHUNK`, which defaults to `MEILI_WRITE_BATCH_SIZE`), write flush interval for the local fallback path (**50** ms, prod **2000**). |
| `MEILI_WRITE_STREAM_ENABLED`, `MEILI_WRITE_STREAM_CONSUMER_ENABLED` | When stream mode is enabled, write-path helpers enqueue Meili upsert/delete operations to Redis Stream `MEILI_WRITE_STREAM_KEY` (the producer stamps an `enqueuedAtMs` field used for the `meili_write_enqueue_to_flush_lag_ms` histogram); leased consumers drain larger cross-worker batches into Meili. If enqueue fails, the worker falls back to local batching/direct delete. |
| `MEILI_WRITE_STREAM_KEY`, `MEILI_WRITE_STREAM_GROUP`, `MEILI_WRITE_STREAM_MAXLEN` | Redis Stream key, consumer group, and approximate max length for cross-worker Meili write batching. Defaults: `meili:messages:write`, `meili-indexers`, **100000**. |
| `MEILI_WRITE_STREAM_CONSUMER_SLOTS`, `MEILI_WRITE_STREAM_READ_COUNT`, `MEILI_WRITE_STREAM_TASK_CHUNK`, `MEILI_WRITE_STREAM_BLOCK_MS`, `MEILI_WRITE_STREAM_COALESCE_MS`, `MEILI_WRITE_STREAM_COALESCE_BLOCK_MS`, `MEILI_WRITE_STREAM_LOCK_TTL_MS`, `MEILI_WRITE_STREAM_CLAIM_IDLE_MS` | Controls the leased Meili stream consumers. Shared prod env can enable consumers on every worker; Redis locks allow only `CONSUMER_SLOTS` active drainers. **Increasing slots requires id-hashed partitioning to preserve per-message ordering — keep at 1 until that exists.** Defaults: **1** slot, read count = `MEILI_WRITE_BATCH_SIZE` (prod **200**), per-task chunk = `MEILI_WRITE_BATCH_SIZE` (prod **200** — bounds documents per single Meili POST after coalescing), **1000 ms** initial block, coalesce window = `MEILI_WRITE_FLUSH_MS` clamped to **0–60000 ms** (prod **2000**), **100 ms** coalesce poll block, **15000 ms** lock TTL, **60000 ms** pending reclaim idle. |
| `MEILI_WRITE_STREAM_REAP_ENABLED`, `MEILI_WRITE_STREAM_REAP_IDLE_MS`, `MEILI_WRITE_STREAM_REAP_INTERVAL_MS` | **QoL / ops:** the lease-holding consumer (slot **0**) periodically runs `XINFO CONSUMERS` and issues `XGROUP DELCONSUMER` for entries that are idle ≥ `REAP_IDLE_MS` (default **3600000** ms), have **pending = 0**, and are not the active consumer name (so no in-flight work is dropped). Throttled to at most once per `REAP_INTERVAL_MS` (default **300000** ms). Disable with `MEILI_WRITE_STREAM_REAP_ENABLED=false`. Counter: `meili_write_stream_consumers_reaped_total`. Stream consumer names use `HOSTNAME` when set, else `os.hostname()` (systemd often omits `HOSTNAME`). |
| `MEILI_TASK_METRICS_ENABLED`, `MEILI_TASK_METRICS_POLL_MIN_MS`, `MEILI_TASK_METRICS_POLL_MAX_MS`, `MEILI_TASK_METRICS_TIMEOUT_MS`, `MEILI_WRITE_QUEUE_DEPTH_INTERVAL_MS` | Async polling of `/tasks/<uid>` to record `meili_task_wait_ms` and `meili_task_duration_ms` (enabled by default; bounded exponential backoff between poll attempts; default timeout 5 min per task). `MEILI_WRITE_QUEUE_DEPTH_INTERVAL_MS` controls how often the consumer samples `XLEN` for the `meili_write_queue_depth` gauge (default **5000 ms**). |
| `MEILI_EMPTY_CANDIDATES_FALLBACK_ENABLED` | When **`true`**, a Meili empty-candidate response still falls back to full Postgres search after the freshness rescue finds nothing. Production keeps this **`false`** once the index is warm: empty candidates return an empty page after the bounded freshness check instead of launching a second expensive FTS/literal pipeline. |
| `MEILI_FRESHNESS_WINDOW_MS`, `MEILI_FRESHNESS_CANDIDATE_LIMIT` | Bounds the primary-DB freshness supplement used to rescue recently edited/deleted messages when Meili is stale. **`MEILI_FRESHNESS_WINDOW_MS=0` disables the supplement**. Positive windows default to **600000 ms** and clamp to **0–3600000 ms**; candidate limit defaults to **15** and clamps to **10–200**. Prod currently disables this path because observed cache reuse was effectively zero and the bounded Postgres scan dominated search p99. |
| **Search (OpenSearch POC)** | |
| `OPENSEARCH_URL`, `OPENSEARCH_USERNAME`, `OPENSEARCH_PASSWORD`, `OPENSEARCH_INDEX_MESSAGES`, `OPENSEARCH_TIMEOUT_MS` | OpenSearch endpoint/auth/index and HTTP timeout for single-document requests (default **8000** ms, clamp **500–30000** ms; bulk backfill uses a separate larger timeout). |
| `OPENSEARCH_DUAL_WRITE_ENABLED`, `OPENSEARCH_READ_ENABLED`, `OPENSEARCH_MAX_CANDIDATES` | POC toggles. Dual-write keeps OpenSearch warm from message writes (fail-open); read flag enables OpenSearch candidate retrieval; max candidates defaults to **250** and clamps to **50–2000**. |
