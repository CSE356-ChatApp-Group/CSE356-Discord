...
`deploy-prod.sh` rewrites the whole `upstream app { ... }` block (no fragile global `sed` on ports). When **`CHATAPP_INSTANCES>=2`**, step **9** only applies **listen backlog / `nginx.conf` / sysctl** tuning and leaves **both** upstreams in place while the candidate warms up. Step **9a** pins nginx to the **candidate port only** before **9b** restarts the companion (**`PIN_CANDIDATE_BEFORE_COMPANION`** defaults **`true`** so POST traffic is not routed to a restarting peer; set `false` only if you accept brief 502s or rely on **`proxy_next_upstream_non_idempotent`** in `/api/`). **9c** restores `least_conn` across both ports with **`max_fails=2 fail_timeout=10s`**. There is still a **shared-traffic window** before **9a** where **old and new code** may both serve requests; keep DB migrations and API responses **backward compatible** across that window. After **9a**, capacity is on the candidate only until **9c** completes.
