...
| Realtime | `redis_fanout_publish_failures_total`, `fanout_publish_duration_ms`, `fanout_publish_targets`, `fanout_target_cache_total`, `conversation_fanout_targets_cache_version_retry_total`, `realtime_passthrough_publish_skipped_total`, `ws_bootstrap_wall_duration_ms`, `ws_backpressure_events_total` | Fanout health, Redis publish multiplier, target-cache effectiveness, conversation fanout cache invalidation races, passthrough publish skips when Redis reports zero subscribers, WS connect bootstrap wall time, and slow clients. |
